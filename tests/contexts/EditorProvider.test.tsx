...
describe('EditorProvider', () => {
  ...
  it('should update tab properties', () => {
    ...
    act(() => {
      result.current.updateTab(tabId, { title: 'Updated Title', query: 'SELECT * FROM users' });
    });
  ...
  it('should get schema from backend on cache miss', async () => {
    ...
    expect(invoke).toHaveBeenCalledWith('get_schema_snapshot', { connectionId: 'conn-1' });
  });
  ...
  it('should use cached schema on subsequent calls', async () => {
    ...
    expect(invoke).toHaveBeenCalledTimes(1); // No additional backend call
    expect(schema).toEqual(mockSchema);
  });
  ...
  it('should only show tabs for active connection', () => {
    ...
    const { result: resultConn1 } = renderHook(() => useEditor(), { wrapper: wrapperConn1 });
    ...
    const { result: resultConn2 } = renderHook(() => useEditor(), { wrapper: wrapperConn2 });
