...
async fn mysql_execute_with_pk(
    ...
    let journalled = qb.sql().to_string();
    let result = if text.enabled {
        exec_stmt(&pool, text, &qb.into_sql()).await?
    } else {
        qb.build().execute(&pool).await.map_err(|e| e.to_string())?
    };
    journal_record_mutation(&pool, params, &journalled).await;
    Ok(result.rows_affected())
}

/// Journals a grid-editor mutation.
///
/// These go straight to the pool through QueryBuilder, so they never reach
/// `execute_query`/`execute_batch` and were invisible to recovery: editing a
/// hundred cells left a hundred writes with no record of the rows they
/// touched. Best-effort, exactly like the batch path — never fails the write.
///
/// When the statement is parameterised the SQL still carries `?` placeholders.
/// That is enough: these are `exact: false` entries whose restore is
/// table-level, and the table name is present either way.
async fn journal_record_mutation(pool: &sqlx::MySqlPool, params: &ConnectionParams, sql: &str) {
    let Ok(mut conn) = pool.acquire().await else {
        return;
    };
    journal_unprotected_changes(&mut conn, params, None, &[(0, sql)]).await;
}
...
/// Journals change statements that ran WITHOUT rollback protection, so the
/// backup-based restore knows which objects were touched. `succeeded` holds
/// (index, sql) of statements that executed successfully; read-only and
/// session statements are filtered out here. Best-effort: a journaling
/// failure is logged, never surfaced — it must not break query execution.
async fn journal_unprotected_changes(
    conn: &mut sqlx::MySqlConnection,
    params: &ConnectionParams,
    schema: Option<&str>,
    succeeded: &[(usize, &str)],
) {
    let (Some(connection_id), Some(connection_name)) = (
    ...
    let changes: Vec<(usize, &str)> = succeeded
        .iter()
        .filter(|(_, sql)| {
            matches!(
                rollback_guard::classify_for_rollback(sql).class,
                rollback_guard::ProtectionClass::SupportedDml
                    | rollback_guard::ProtectionClass::SupportedDdl
                    | rollback_guard::ProtectionClass::BlockedDestructive
                    | rollback_guard::ProtectionClass::BlockedUnsupported
            )
        })
        .copied()
        .collect();
    ...
    let database = schema.unwrap_or_else(|| params.database.primary());
    if let Err(error) = crate::recovery_history::record_unprotected_changes(
        conn,
        connection_id,
        connection_name,
        database,
    ...
    .await
    {
        log::warn!("Could not journal unprotected changes: {error}");
    }
}
