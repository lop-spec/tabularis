...
fn cancelled_batch_statement_index(
    completed_statements: &[std::sync::atomic::AtomicBool],
) -> Option<usize> {
    completed_statements
        .iter()
        .position(|completed| !completed.load(std::sync::atomic::Ordering::Acquire))
}

fn query_join_outcome<T>(result: Result<T, tokio::task::JoinError>) -> Result<T, String> {
    match result {
        Ok(value) => Ok(value),
        Err(error) if error.is_cancelled() => Err("Query cancelled".to_string()),
        Err(error) => Err(format!("Query execution task failed: {error}")),
    }
}

enum QueryAuditAppendContext<'a> {
    Success,
    Failed(&'a str),
    Cancelled,
}

fn require_query_audit_append(
    context: QueryAuditAppendContext<'_>,
    append_result: Result<(), String>,
) -> Result<(), String> {
    append_result.map_err(|error| match context {
        QueryAuditAppendContext::Success => {
            format!("Query succeeded but its audit event could not be persisted: {error}")
        }
        QueryAuditAppendContext::Failed(original_error) => format!(
            "Query failed and its audit event could not be persisted: {error}; original error: {original_error}"
        ),
        QueryAuditAppendContext::Cancelled => {
            format!("Query was cancelled but its audit event could not be persisted: {error}")
        }
    })
}
...
/// Resolves with both the id and the name. Recovery journalling requires the
/// name, so any path that performs writes has to use this rather than
/// [`resolve_connection_params_with_id`], or its changes go unrecorded.
pub fn resolve_connection_params_with_identity(
    params: &ConnectionParams,
    connection_id: &str,
    connection_name: &str,
) -> Result<ConnectionParams, String> {
...
pub async fn test_connection<R: Runtime>(
    ...
    let result: Result<String, String> = async {
        ...
        drv.test_connection(&resolved_params)
            .await
            .map(|()| "Connection successful!".to_string())
    }
    .await;
...
mod tests {
    ...
    mod cancellation_state {
        use super::super::{
            cancel_query_impl, cancelled_batch_statement_index, query_join_outcome,
            register_abort_handle, require_query_audit_append, unregister_abort_handle,
            QueryAuditAppendContext, QueryCancellationState,
        };
        use std::sync::{
        ...
        #[test]
        fn cancelled_batch_index_uses_callback_progress_not_audit_event_count() {
            let completed = [AtomicBool::new(false), AtomicBool::new(false)];

            // A protectively skipped statement still completed its callback,
            // even though it intentionally produced no audit event.
            completed[0].store(true, Ordering::Release);

            assert_eq!(cancelled_batch_statement_index(&completed), Some(1));
        }

        #[test]
        fn cancelled_batch_index_is_none_after_every_callback_completed() {
            let completed = [AtomicBool::new(true), AtomicBool::new(true)];

            assert_eq!(cancelled_batch_statement_index(&completed), None);
        }

        #[test]
        fn query_audit_append_failures_preserve_database_outcome() {
            let success = require_query_audit_append(
                QueryAuditAppendContext::Success,
                Err("outbox unavailable".to_string()),
            )
            .unwrap_err();
            let failed = require_query_audit_append(
                QueryAuditAppendContext::Failed("database rejected statement"),
                Err("outbox unavailable".to_string()),
            )
            .unwrap_err();
            let cancelled = require_query_audit_append(
                QueryAuditAppendContext::Cancelled,
                Err("outbox unavailable".to_string()),
            )
            .unwrap_err();
            ...
            assert_eq!(
                success,
                "Query succeeded but its audit event could not be persisted: outbox unavailable"
            );
            assert_eq!(
                failed,
                "Query failed and its audit event could not be persisted: outbox unavailable; original error: database rejected statement"
            );
            assert_eq!(
                cancelled,
                "Query was cancelled but its audit event could not be persisted: outbox unavailable"
            );
            assert_eq!(
                require_query_audit_append(QueryAuditAppendContext::Success, Ok(())),
                Ok(())
            );
            assert_eq!(
                require_query_audit_append(
                    QueryAuditAppendContext::Failed("database rejected statement"),
                    Ok(())
                ),
                Ok(())
            ...
            assert_eq!(
                require_query_audit_append(QueryAuditAppendContext::Cancelled, Ok(())),
                Ok(())
            );
        }

        #[tokio::test]
        async fn task_panic_is_not_reported_as_cancelled() {
            let task = tokio::spawn(async {
                panic!("database task panic");
            });

            let error = query_join_outcome(task.await).unwrap_err();
            assert!(error.starts_with("Query execution task failed:"));
            assert_ne!(error, "Query cancelled");
        }

        #[tokio::test]
        async fn abort_after_database_task_completed_keeps_completed_result() {
            let task = tokio::spawn(async { "database result" });
            ...
            assert_eq!(query_join_outcome(task.await).unwrap(), "database result");
        }

        #[tokio::test]
        async fn abort_before_database_task_completed_is_cancelled() {
            let task = spawn_sleeper().await;
            task.abort();

            assert_eq!(
                query_join_outcome(task.await).unwrap_err(),
                "Query cancelled"
            );
        }
...
pub async fn delete_record<R: Runtime>(
    ...
    let expanded_params = expand_ssh_connection_params(&app, &saved_conn.params).await?;
    let expanded_params = expand_k8s_connection_params(&app, &expanded_params).await?;
    // With the name: recovery journalling is gated on it, so a grid edit
    // resolved by id alone left no record of the row it changed.
...
/// Session statements to replay for a window, or `None` when the caller did not
/// identify a window (sidebar previews, modals, background lookups) — those
/// keep running with a clean session, which is what they expect.
fn session_preamble_for(
    connection_id: &str,
    session_context_id: Option<&str>,
) -> Option<crate::session_vars::SessionPreamble> {
    crate::session_vars::preamble(connection_id, session_context_id?)
}

/// Session statements currently in force for one editor window, in replay
/// order, so the UI can show and reset them.
#[tauri::command]
pub fn list_session_variables(
    connection_id: String,
    session_context_id: String,
) -> Result<Vec<String>, String> {
    Ok(crate::session_vars::list_window(
        &connection_id,
        &session_context_id,
    ))
}

/// Forgets a window's session statements (tab closed or reset), or every window
/// of the connection when no window is given (disconnect).
#[tauri::command]
pub fn clear_session_variables(
    connection_id: String,
    session_context_id: Option<String>,
) -> Result<(), String> {
    match session_context_id {
        Some(context) => crate::session_vars::clear_window(&connection_id, &context),
        None => {
            let cleared = crate::session_vars::clear_connection(&connection_id);
            log::info!("Cleared session state of {cleared} window(s) on {connection_id}");
        }
    }
    Ok(())
}
...
pub async fn execute_query<R: Runtime>(
    ...
    let saved_conn = find_connection_by_id(&app, &connection_id)?;
    let audit_target = crate::audit_outbox::audit_target(&saved_conn, schema.as_deref());
    let expanded_params = expand_ssh_connection_params(&app, &saved_conn.params).await?;
    let expanded_params = expand_k8s_connection_params(&app, &expanded_params).await?;
    // Carries the connection name, not just the id: recovery journalling is
    // gated on the name being present, so resolving without it made
    // `journal_unprotected_changes` a no-op for every single-statement run —
    // editor Run, sidebar DROP TABLE, column changes, notebooks.
    let mut params = resolve_connection_params_with_identity(
        &expanded_params,
        &connection_id,
        &saved_conn.name,
    )?;
    // Statements the calling window ran earlier are replayed on whatever
    // connection the pool hands out, so `SET @var` survives being executed on
    // its own instead of as part of a batch.
    ...
    let audit_sql = sanitized_query.clone();

    let drv = driver_for(&saved_conn.params.driver).await?;
    let audit_started_epoch_us = crate::audit_outbox::now_epoch_us();
    ...
    match query_join_outcome(result) {
        Ok(Ok(query_result)) => {
            log::info!(
            ...
            if let Some(target) = &audit_target {
                let event = crate::audit_outbox::AuditEvent::new(
                    target,
                    &audit_sql,
                    audit_started_epoch_us,
                    crate::audit_outbox::now_epoch_us(),
                    crate::audit_outbox::AuditExecutionStatus::Success,
            ...
            if let Some(context) = &session_context_id {
                crate::session_vars::record_statement(&connection_id, context, &audit_sql);
            }
        ...
        Ok(Err(e)) => {
            ...
            if let Some(target) = &audit_target {
                let event = crate::audit_outbox::AuditEvent::new(
                    ...
                    audit_started_epoch_us,
                    crate::audit_outbox::now_epoch_us(),
                    crate::audit_outbox::AuditExecutionStatus::Failed,
                    0,
                    &e,
                    None,
                    None,
                ...
                require_query_audit_append(
                    QueryAuditAppendContext::Failed(&e),
                    crate::audit_outbox::append_required(&app, &event),
                )?;
            }
            Err(e)
        ...
        Err(e) if e == "Query cancelled" => {
            ...
            if let Some(target) = &audit_target {
                let event = crate::audit_outbox::AuditEvent::new(
                    ...
                    audit_started_epoch_us,
                    crate::audit_outbox::now_epoch_us(),
                    crate::audit_outbox::AuditExecutionStatus::Cancelled,
                    0,
                    &e,
                    None,
                    None,
...
pub async fn execute_query_batch<R: Runtime>(
    ...
    let audit_queries = sanitized_queries.clone();
    let audit_event_ids: Vec<String> = sanitized_queries
        .iter()
        .map(|_| crate::audit_outbox::new_event_id())
        .collect();
    let audit_batch_id = batch_id.clone();
    let audit_transaction_context_id = transaction_context_id.clone();
    ...
    params.allow_implicit_transaction_commit = allow_implicit_transaction_commit.unwrap_or(false);
    // The batch already shares one connection, so the preamble only has to
    // restore what earlier runs of this window set — statements inside the
    // batch keep seeing each other's `SET`s as before.
    params.session_preamble = session_preamble_for(&connection_id, session_context_id.as_deref());

    let drv = driver_for(&saved_conn.params.driver).await?;

    // Build a Tauri-agnostic progress sink the driver invokes per statement.
    // Audit persistence is attempted before the UI event is emitted so the
    // callback cannot report an executed statement while silently dropping its
    // durable audit identity.
    let completed_statements = Arc::new(
        (0..audit_queries.len())
            .map(|_| std::sync::atomic::AtomicBool::new(false))
            .collect::<Vec<_>>(),
    );
    let progress: Option<Arc<crate::drivers::driver_trait::BatchProgressFn>> =
        if audit_target.is_some() || batch_id.is_some() {
            let app = app.clone();
            let target = audit_target.clone();
            let queries = audit_queries.clone();
            let event_ids = audit_event_ids.clone();
            let audit_batch_id = audit_batch_id.clone();
            let transaction_context_id = audit_transaction_context_id.clone();
            let ui_batch_id = batch_id.clone();
            let completed = completed_statements.clone();
            let cb: Arc<crate::drivers::driver_trait::BatchProgressFn> =
                Arc::new(move |index, statement: &BatchStatementResult| {
                    if let Some(target) = &target {
                        crate::audit_outbox::persist_statement_outcome(
                            &app,
                            &event_ids[index],
                            target,
                            &queries[index],
                            statement,
                            audit_batch_id.as_deref(),
                            index,
                            transaction_context_id.as_deref(),
                        )?;
                    }
                    completed[index].store(true, std::sync::atomic::Ordering::Release);
                    if let Some(bid) = &ui_batch_id {
                        let _ = app.emit(
    ...
    match result {
        Ok(Ok(batch_results)) => {
            ...
            if let Some(context) = &session_context_id {
                // Only statements that actually ran: a skipped or failed `SET`
                // never took effect, so replaying it would invent state.
                for (sql, statement) in audit_queries.iter().zip(&batch_results) {
                    if statement.error.is_none() && statement.skipped != Some(true) {
                        crate::session_vars::record_statement(&connection_id, context, sql);
                    }
                }
            }
            Ok(batch_results)
        ...
        Err(join_error) if join_error.is_cancelled() => {
            ...
            if let Some(target) = &audit_target {
                let cancelled = "Query cancelled";
                let finished_epoch_us = crate::audit_outbox::now_epoch_us();
                if let Some(index) = cancelled_batch_statement_index(&completed_statements) {
                    let event = crate::audit_outbox::AuditEvent::with_planned_id(
                        &audit_event_ids[index],
                        target,
                        &audit_queries[index],
                        finished_epoch_us,
                        finished_epoch_us,
                        ...
                        audit_batch_id.as_deref(),
                        Some(index),
                        audit_transaction_context_id.as_deref(),
                    );
                    crate::audit_outbox::append_required(&app, &event).map_err(|error| {
                        format!(
                            "Batch was cancelled but its audit event could not be persisted: {error}"
                        )
                    })?;
                }
            }
        ...
        Err(join_error) => {
            log::error!("Batch execution task failed: {join_error}");
            Err(format!("Batch execution task failed: {join_error}"))
        }
    }
}
...
pub async fn explain_query_plan<R: Runtime>(
    ...
    // EXPLAIN of a query that reads `@var` needs the window's session state as
    // much as executing it does.
    params.session_preamble = session_preamble_for(&connection_id, session_context_id.as_deref());
...
pub async fn count_query<R: Runtime>(
    ...
    let expanded_params = expand_k8s_connection_params(&app, &expanded_params).await?;
    let mut params = resolve_connection_params_with_id(&expanded_params, &connection_id)?;
    // Counting wraps the window's own query, so it needs the same session
    // state — otherwise a query filtering on `@cutoff` counts against NULL.
    params.session_preamble = session_preamble_for(&connection_id, session_context_id.as_deref());
...
pub async fn rollback_transaction_context(
    ...
    crate::drivers::mysql::rollback_transaction_context(&connection_id, &transaction_context_id)
        .await
}
...
pub async fn disconnect_connection<R: Runtime>(
    ...
    // The pool is about to go away, so no remembered `SET` can still be in
    // force; keeping them would silently reapply stale values after reconnect.
    let cleared_windows = crate::session_vars::clear_connection(&connection_id);
    if cleared_windows > 0 {
        log::info!("Cleared session state of {cleared_windows} window(s) on {connection_id}");
    }
